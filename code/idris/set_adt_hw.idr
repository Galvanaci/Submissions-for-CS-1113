...
-- Return true p is true for every v in s, otherwise false, every
  -- element in this set have some property
set_forall: (p: a -> bool) -> (s: set a) -> bool


-- Return true if p is true for some v in s, otherwise else false
-- only one element in the set needs this property
...
-- call it a "witness to the property p." Returns an element in the
...
list_remove v l = filter (notmember v) l
...
--takes two lists and gives a copy of the first list minus the
  --elements that are in the second list
filter_member: (eq a) => list a -> list a -> list a
filter_member nil nil = nil
filter_member l1 nil = l1
filter_member nil l2 = nil
filter_member (h::t) l2 = ite (member h l2) (filter_member t l2) (h:: (filter_member t l2))

--takes two lists and appends them, while removing the elements from
  --the first list that exist in the second.
...
--given two lists, return a list of elements that the two lists had in common
...
--given two lists, returns a list containing only elements that were
...
-- Return the set difference, s1 minus s2
--set_difference: (eq a) => (s1: set a) -> (s2: set a) -> set a
set_difference (mkSet l1) (mkSet l2) = mkSet ((list_difference l1 l2) ++ (list_difference l2 l1))
...
list_forall p (h::t) = ite (p h) (list_forall p t) (false)
...
list_exists p (h::t) = ite (p h) (true) (list_exists p t)
...
--- pairelt takes an element h and a list b and returns a list of all
